...
module VLog =
    ...
    let saveState (ts: TestSetup) =
        let str x = String.concat " " x + "\n"
        let nameLine = [ts.Name]
        let state dp =
            let regs = "Regs" :: (dp.TRegs |> List.map (sprintf "0x%x"))
            let flags = 
                let f = dp.TFlags
                "NZCV" :: (
                    [f.FN ; f.FZ ; f.FC ; f.FV] 
                    |> List.map ((function | true -> 1 | false -> 0) >> sprintf "%d"))
            [regs ;  flags]
        let afterMess = 
            match ts.After with
            | Some after -> state after
            | None -> [["ERROR"]]
        [nameLine] @ (state ts.Before) @ [["..."] ; [ts.Asm] ; ["..."]] @ afterMess
        |> List.map str
        |> String.concat ""
